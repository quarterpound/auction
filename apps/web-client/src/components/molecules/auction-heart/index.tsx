...
const AuctionHeart = ({id, count}: AuctionHeartProps) => {
  ...
  return (
    <Button className="flex items-center gap-2" disabled={mutation.isPending} size={actualCount === 0 ? 'icon' : 'default'} variant={'outline'} onClick={() => mutation.mutateAsync({id})}>
      <Heart className={cn("w-5 h-5 flex-shrink-0", {
        "fill-red-500 stroke-red-500": isFavorited,
      })} />
      {actualCount !== 0 && <span className="font-semibold">{`Favorited by ${actualCount} users`}</span>}
    </Button>
  )
}
